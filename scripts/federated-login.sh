...
IDP_TOKEN=`openstack token issue -f value -c id`
if [ "$?" != "0" ]; then
    echo "Could not obtain IdP token. (Did you forget to import your openrc file?)"
    exit 1
fi
echo - Obtained IdP token.

# obtain service provider's URLs
SP_URL=`openstack service provider show $SP_ID -c sp_url -f value`
if [ "$?" != "0" ]; then
    echo "Could not find service provider $SP_ID."
    exit 1
fi
SP_AUTH_URL=`openstack service provider show $SP_ID -c auth_url -f value`
if [ "$?" != "0" ]; then
    echo "Could not find service provider $SP_ID."
    exit 1
fi
...
if [ "$?" != "0" ] || grep -q error assertion.xml; then
    echo Coult not obtain SAML2 assertion. See assertion.xml and error.log for details.
    exit 1
fi
...
# cleanup
rm assertion.xml cookies.txt unscoped.txt scoped.txt catalog.txt error.log _print_vars.py fed_projects.json
